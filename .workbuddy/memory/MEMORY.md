...
- 答题模式：手动(点下一题) / 自动(倒计时结束3秒后自动下一题)；科目答题开关(开启选科目/关闭随机打乱)
- 主题：5 套(深紫/深蓝/暖橙/赛博绿/玫红)，html[data-theme] 切换 CSS 变量
- 配置导入/导出：仅含设置不含题目，JSON 格式（导出时排除 API Key）
- AI 生成题目：OpenAI 兼容接口，配置 baseURL/APIKey/Model，按科目/数量/答案分布生成，预览审核后导入题库
